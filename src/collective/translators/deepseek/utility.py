...
class DeepSeekFactory:
    ...
    @property
    def translator(self):
        key = api.portal.get_registry_record("collective.translators.interfaces.IDeepSeekControlPanel.api_key")
        client = OpenAI(api_key=key, base_url=self.server_url)
        return client
    
    def is_available(self):
        value = api.portal.get_registry_record("collective.translators.interfaces.IDeepSeekControlPanel.enabled")
        return value
    ...
    def translate_content(self, content, source_language, target_language):
        try:
            ...
            api.portal.show_message(message=f"Translation error: {str(e)}", request=api.portal.get().REQUEST, type='error')
            return None
